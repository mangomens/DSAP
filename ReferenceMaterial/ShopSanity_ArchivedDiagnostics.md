# Shop Sanity — Archived Diagnostic & Research Code

> **Archived 2026-04-02.** These methods were development/debugging tools used during Shop Sanity implementation.
> They are preserved here for reference in case similar diagnostics are needed for future shop phases.
> Originally located in `ShopHelper.cs` and `ApItemInjectorHelper.cs`.

## Client Commands (were available via / commands in DSAP client)

| Command | Method | Purpose |
|---------|--------|---------|
| `/shopdiag` | `DumpShopDiagnostics()` | Memory state vs expected replacement map |
| `/paraminfo` | `DumpParamInfo()` | Param table metadata + MsgMan FMG scan |
| `/goodstype` | `DumpGoodsTypeAnalysis()` | EquipParamGoods field analysis |
| `/shopflags` | `DumpShopEventFlags()` | Event flag bit states + shared-byte analysis |
| `/shopraw` | `DumpShopRawBytes()` | Raw 0x20 hex dump of every merchant row |
| `/shopstruct` | `DumpShopStructure()` | Binary search table + endTable verification |
| `/shopvis` | `DumpShopVisibility()` | Item resolution check in equip param tables |
| `/shopvanilla` | `DumpVanillaComparison()` | Vanilla vs modified param side-by-side |

## Dead Stub Methods (from ApItemInjectorHelper.cs)

These methods created weapon/protector/accessory param stubs for shop items.
Abandoned because weapon/protector FMGs are inaccessible via MsgMan, so items
had no names in the shop UI. Replaced by native passthrough approach.

---

## ShopHelper.cs Diagnostic Methods
...
## ApItemInjectorHelper.cs Dead Stub Methods
